...
import static io.fnproject.example.FnClientCommon.loadPrivateKey;
...
public class FnHTTPPost {
    ...
    public <T> T invoke(String endpoint, HttpEntity payload, ResponseHandler<? extends T> rh) {
        ...
        System.out.println("Fn endpoint - " + endpoint);
        System.out.println("Header - " + request.getFirstHeader("Authorization"));

        /**
         * Apache HTTP client adds Content-Type header again resulting in -
         * org.apache.http.ProtocolException: Content-Length header already
         * present. Added interceptor to remove that header since we already add
         * it in signRequest method*
         */
